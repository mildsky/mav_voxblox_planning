...
class OnlineSkeletonizerNode {
   public:
    OnlineSkeletonizerNode(const ros::NodeHandle& nh, const ros::NodeHandle& nh_private)
        : nh_(nh),
        ...
        nh_private_.param("update_rate", update_rate_, update_rate_);
        skeleton_timer_ = nh_.createTimer(ros::Duration(1.0 / update_rate_), &OnlineSkeletonizerNode::updateSkeleton, this);
    }
    ...
    //   // Make a skeletor!!!
    void skeletonize(Layer<EsdfVoxel>* esdf_layer, voxblox::Pointcloud* skeleton, std::vector<float>* distances);

    void updateSkeleton(const ros::TimerEvent& event);
...
void OnlineSkeletonizerNode::updateSkeleton(const ros::TimerEvent& event) {
    esdf_server_.updateMesh();
    esdf_server_.publishPointclouds();
    esdf_server_.publishMap();

    if (!esdf_server_.getEsdfMapPtr()) {
        ROS_WARN("ESDF map not yet available.");
        return;
    }

    // Get ESDF Layer
    Layer<EsdfVoxel>* esdf_layer = esdf_server_.getEsdfMapPtr()->getEsdfLayerPtr();
    if (!esdf_layer || esdf_layer->getNumberOfAllocatedBlocks() == 0) {
        ROS_WARN("No allocated ESDF blocks found. Skipping skeletonization.");
        return;
    }
    ROS_INFO("Start skeletonize ESDF.");
    voxblox::Pointcloud pointcloud;
    std::vector<float> distances;
    skeletonize(esdf_layer, &pointcloud, &distances);
    ...
    // Generate and publish sparse graph
    // skeleton_generator_.generateSparseGraph();
    // visualization_msgs::MarkerArray marker_array;
    // visualizeSkeletonGraph(skeleton_generator_.getSparseGraph(), frame_id_, &marker_array);
    // sparse_graph_pub_.publish(marker_array);

    ROS_INFO("Published online skeleton and sparse graph.");
}

void OnlineSkeletonizerNode::init() {
...
void OnlineSkeletonizerNode::skeletonize(Layer<EsdfVoxel>* esdf_layer, voxblox::Pointcloud* pointcloud, std::vector<float>* distances) {
    skeleton_generator_.setEsdfLayer(esdf_layer);

    FloatingPoint min_separation_angle = skeleton_generator_.getMinSeparationAngle();
...
int main(int argc, char** argv) {
    ...
    voxblox::OnlineSkeletonizerNode node(nh, nh_private);
